...
date: 2019-08-04T10:21:55+00:00
lastmod: 2019-08-04T10:48:35+00:00
---

Ubuntu 16.04 comes with a pretty old version of GNU GPG. I was trying to setup GPG key for my Github account. On checking the version, I saw the following -
...
Github provides instructions on how to do it with older versions as well but I thought, lets install the latest version. In order to get the latest version, we need to download the packages, build them and then install them.

# Pre-Requisites

Before proceeding ahead, ensure that you have installed the `build-essential`,`zlib1g-dev` `make` packages. Also, ensure that you have `sudo` privilleges. These can be installed by typing in -

`sudo apt-get install build-essential make zlib1g-dev`

# Downloading the latest versions

The latest versions of the packages are available over [here](https://www.gnupg.org/download/). As of writing this, the latest versions are as below -
...
For the below instructions, if you notice that the versions have changed, you would have to edit the commands accordingly.
...
When you run `gpg --full-generate-key` and fill in the options, you will get an error like below -
...
We need to install `pinentry` so that gpg can generate the key. Install & configure Pinentry using the following commands -
