...
date: 2019-08-22T12:23:10+00:00
lastmod: 2019-08-22T12:23:10+00:00
---

Howdy!

I was doing Splunk development (On Windows) for External Lookups when I encountered this issue. I like to turn off the Splunkd service so that it doesn't keep on consuming my precious resources.

I started the service like normal after a few days and I saw the following after logging in -

![Splunk-KV-Store-Failed](/content/images/2019/08/Splunk-KV-Store-Failed.png)

This was pretty strange as to why it started happening all of a sudden when it was working previously. I started going through the logs as mentioned in the screenshot (splunkd.log & mongod.log). These logs are located in `%SPLUNK_HOME%/var/log/splunk`.
...
As you can see, the Mongo DB instance is not able to access the file located in `C:\Program Files\Splunk\var\lib\splunk\kvstore\mongo\journal\lsn`

I checked the permissions on the **kvstore\mongo\journal** folder (& the objects within) and strangely I saw only my user who had the permissions.

![Journal-File-Permissions](/content/images/2019/08/Journal-File-Permissions.png)

Well, now I know the problem, I turned out to Process Monitor (Procmon!) to see who was the user which was being used by Splunk to access this file -

![Procmon-trace](/content/images/2019/08/Procmon-trace.png)

After seeing that, I gave full permissions on this folder - `%SPLUNK_HOME%\var\lib\splunk\kvstore\mongo\` to the SYSTEM user and restarted Splunk which fixed the issue!
